...
namespace Challenges
{
    public class SherlockMaze
    {
        ...
        /// <created>Andrew Haselden, andrewhaselden@gmail.com ,4/9/2019</created>
        /// <changed>Andrew Haselden, andrewhaselden@gmail.com ,4/9/2019</changed>
        private void SearchPaths(Node current, Node previous, string currentPath, int turns)
        {
            ...
            if (current.N == DestinationNode.N && current.M == DestinationNode.M)
            {
                ...
                currentPath += $"{current.N},{current.M}_";
                SuccessfulPaths++;
                return;
            }
            ...
            {
                SearchPaths(nextNodeRight, new Node(current.N, current.M), currentPath, turns+1);
            }
            else 
                SearchPaths(nextNodeRight, new Node(current.N, current.M), currentPath, turns);

            if (previous.M == current.M)
            {
                SearchPaths(nextNodeDown, new Node(current.N, current.M), currentPath, turns + 1);
            }
            else
                SearchPaths(nextNodeDown, new Node(current.N, current.M), currentPath, turns);
        ...
        public string Run()
        {
            ...
            SearchPaths(new Node(StartingNode.N + 1, StartingNode.M), StartingNode, "1,1_", 0);
            SearchPaths(new Node(StartingNode.N, StartingNode.M + 1), StartingNode, "1,1_", 0);
